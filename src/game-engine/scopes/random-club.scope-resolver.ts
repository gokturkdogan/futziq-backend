...
function resolveClubId(definition: ScopeDefinition, context: ScopeContext): string | null {
  const fromDefinition = definition.params?.clubId;
  if (typeof fromDefinition === 'string' && fromDefinition.length > 0) {
    return fromDefinition;
  }
  const fromContext = context.scopeParams?.clubId;
  return typeof fromContext === 'string' && fromContext.length > 0 ? fromContext : null;
}

@Injectable()
export class RandomClubScopeResolver implements ScopeResolver {
  readonly code = ScopeCode.RANDOM_CLUB;
  ...
  async searchEligiblePlayers(
    ...
    const clubId = resolveClubId(definition, context);
    if (!clubId) {
  ...
  async isPlayerEligible(
    ...
    const belongsToClub = await this.footballData.isPlayerInClub(playerId, clubId);
    if (!belongsToClub) {
      return false;
    }

    return this.validateMetricAndSlot(playerId, context, clubId);
  }

  private async validateMetricAndSlot(
    playerId: string,
    context: ScopeContext,
    _clubId: string,
  ): Promise<boolean> {
